...
int main(int argc, char *argv[])
{
    ...
    {
        boost::asio::post(pool, [&node, &unauth_node, &last_legitimate_fcu, response, request]()
                          {
    ...
    {
        boost::asio::post(pool, [&node, &unauth_node, &last_legitimate_fcu, &jwt, &fee_override_chance, &fee_override_address, response, request]()
                          {
            std::string body = request->content.string();
            json j = json::parse(body);
